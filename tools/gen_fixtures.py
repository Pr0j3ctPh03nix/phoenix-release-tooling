...
def build():
    ...
    out.append(("schema2-options.json", schema2_options(), "accept",
                "An unbundled document — schema 2, every entry named, no bundles. Schema 2 is IN "
                "RANGE, so a reader that handles only bundled documents is not conforming, however "
                "recently it was written. Which releases are actually served in this shape is a "
                "deployment question, and not one this suite answers."))
